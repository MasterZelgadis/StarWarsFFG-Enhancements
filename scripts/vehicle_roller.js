...
export async function intercept_vehicle_roll(...args) {
    ...
    if (possible_actors.length > 1) {
        ...
        args[0].actor = game.actors.get(actor.actor_id).data;
        ...
        // update the entity so the chat shows it properly
        args[0].actor = game.actors.get(possible_actors[0].id).data;
    }
    return args;
}
...
class SelectVehicleActor extends FormApplication {
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            title: game.i18n.localize('ffg-star-wars-enhancements.vehicle-roller-actor-select-title'),
        });
    }
...
function get_dice_pool(actor_id, skill_name, incoming_roll) {
    ...
    var skill = actor.data.data.skills[skill_name];
...
function get_actors(vehicle) {
    ...
    if (actors.length === 0) {
        ...
        actors = game.actors.filter(actor => actor.owner === true && actor.data.type !== 'vehicle');
